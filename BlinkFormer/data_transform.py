...
class ColorJitter(object):
	...
	def __call__(self, imgs):
		print(imgs.shape)
		if imgs.ndim == 3:
			print("image shape channel is 3, use color jittor")
...
def transforms_eval(img_size=224,
					...
					mean=IMAGENET_DEFAULT_MEAN,
					std=IMAGENET_DEFAULT_STD):
	crop_pct = crop_pct or DEFAULT_CROP_PCT # crop_pct: 中心裁剪图像的百分比。默认为None，这将使其使用默认的裁剪百分比。
	...
	tfl = [
		transforms.Resize(scale_size, interpolation=str_to_interp_mode(interpolation)), # 先放大再中心裁剪
...
if __name__ == '__main__':
	# test argumentation for video
	# args = parse_args()

	# hust = HUSTDataModule(configs=args)
	import data_transform as T
	...
	hust_train_data = HUSTDataset(transform=transform)
	from torch.utils.data import DataLoader, Dataset
	hust_train_dataloader = DataLoader(hust_train_data, batch_size=4, shuffle=True, num_workers=0, drop_last=False)

	# clear all the files in the folder
	import shutil
	if os.path.exists("./vis_temp/"):
		shutil.rmtree("./vis_temp/")

	for batch in hust_train_dataloader:
		eye_images, labels, frames_orig, img_folders, annos = batch
		print(eye_images.shape)
		# visualize eye images, save each image frame by frame
		for i in range(eye_images.shape[0]):
			eye_video = eye_images[i]
			eye_video_orig = frames_orig[i]
			label = labels[i]

			for j in range(eye_video.shape[0]):
				eye_frame = eye_video[j]
				eye_frame_orig = eye_video_orig[j].numpy()
				eye_frame = eye_frame.permute(1, 2, 0).numpy().astype(np.uint8)

				# horziontal  concate original frame and transformed frame

				# eye_frame_orig = eye_frame_orig.permute(1, 2, 0).numpy()

				# print data type
				# print(eye_frame_orig.dtype) uint8
				# print(eye_frame.dtype)

				eye_frame = cv2.hconcat([eye_frame_orig, eye_frame])
				
				save_path = "./vis_temp/hust_{}_label_{}/".format(i, label)
				if not os.path.exists(save_path):
					os.makedirs(save_path)
				
				cv2.imwrite("{}/eye_frame_{}.jpg".format(save_path, j), eye_frame)
		exit()

	# transform = create_video_transform(input_size=48, is_training=True)
